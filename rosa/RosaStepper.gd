...
# Based on https://github.com/aexoden/rosa/blob/f2aeb34c4c16a4a0cc0525236730cbeb4ed31019/src/state.hh#L11
func _init(rate: int, initial_seed: int):
	self.step_seed = initial_seed
	self.encounter_seed = (initial_seed * 2) % 256
	self.encounter_rate = rate
	self.step_index = 0
	self.encounter_index = 0

# Hack around Godot limitation where making copies is a pain
func set_seeds_from(other: RefCounted):
	self.step_seed = other.step_seed
	self.step_index = other.step_index
	self.encounter_seed = other.encounter_seed
	self.encounter_index = other.encounter_index

# Based on https://github.com/aexoden/rosa/blob/f2aeb34c4c16a4a0cc0525236730cbeb4ed31019/src/engine.cc#L497
func step() -> int:
	step_index = (step_index + 1) % 256
	if step_index == 0:
		step_seed = (step_seed + 17) % 256
	
	if _rng(step_index, step_seed) < encounter_rate:  
		# Yes, this does seem to be rolling the rng other way round confusingly.
		var encounter_rng = _rng(encounter_index, encounter_seed)
		encounter_index = (encounter_index + 1) % 256
		if encounter_index == 0: 
			encounter_seed = (encounter_seed + 17) % 256

		if (encounter_rng < 43): 
			return 0
		elif (encounter_rng < 86):
			return 1
		elif (encounter_rng < 129):
			return 2
		elif (encounter_rng < 172):
			return 3
		elif (encounter_rng < 204):
			return 4
		elif (encounter_rng < 236):
			return 5
		elif (encounter_rng < 252):
			return 6
		else:
			return 7
...
static func _rng(i: int, s: int) -> int:
	return (_rng_data[i] + s) % 256

# Based on https://github.com/aexoden/rosa/blob/f2aeb34c4c16a4a0cc0525236730cbeb4ed31019/src/engine.hh#L56
static var _rng_data: Array = [ 0x07, 0xB6, 0xF0, 0x1F, 0x55, 0x5B, 0x37, 0xE3, 0xAE, 0x4F, 0xB2, 0x5E, 0x99, 0xF6, 0x77, 0xCB,
